...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_laser", setLaser); // Only for VIP
	...
	RegConsoleCmd("sm_awp", giveAWP, ""); // Only for VIP
...
Action setLaser(int client, int args) // Only for VIP
{
	...
	else
	{
		...
		if (limits[client] > 4) { CPrintToChat(client, "%t", "Limits"); return Plugin_Handled; } else { limits[client]++; }
