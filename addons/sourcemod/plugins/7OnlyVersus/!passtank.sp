	...
	if (!IsAllowToPass(tank)) {
		if (hasTank) {
			if (g_iCvarPassedCount == 1)
				PrintToChat(client, "%t", "phrase2"); // Танка можно передать только на первом контроле."
			else
				PrintToChat(client, "%t", "phrase13", g_iPassedCount[tank], g_iCvarPassedCount); //Достигнут лимит передачи Танка [{1}/{2."
		}
		if (validate == Validate_NotiyfyTarget && hasTarget) {
			if (g_iCvarPassedCount == 1)
				PrintToChat(client, "%t", "phrase2"); // Танка можно передать только на первом контроле."
