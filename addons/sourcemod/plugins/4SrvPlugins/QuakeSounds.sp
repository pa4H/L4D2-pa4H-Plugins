...
public EventPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (attackerClient > 0 && attackerClient <= iMaxClients)
	{
		...
		else
		{
			...
			if (tempLastKillTime == -1.0 || (lastKillTime[attackerClient] - tempLastKillTime) > 3)
			{
...
public PlayQuakeSound(soundKey, killsValue, attackerClient, victimClient)
{
	...
	if (config & 1)
	{
		if (IsClientInGame(attackerClient) && !IsFakeClient(attackerClient) && soundPreference[attackerClient] > -1)
		{
			...
			switch (filePosition)
			{
				case 0:
				{
					PrintCenterTextAll("%N IS FUCKAGE!", attackerClient);
				}
				case 1:
				{
					PrintCenterTextAll("%N IS ON FUCKING SPREE!", attackerClient);
				}
				case 2:
				{
					PrintCenterTextAll("%N IS MONSTER FUCK-ASS!", attackerClient);
				}
				case 3:
				{
					PrintCenterTextAll("LUDICROUS MASTURBATION BY %N!", attackerClient);
				}
				case 4:
				{
					PrintCenterTextAll("%N HOLY FUCK!", attackerClient);
				}
				case 5:
				{
					PrintCenterTextAll("%N IS UNFUCKABLE!", attackerClient);
				}
				case 6:
				{
					PrintCenterTextAll("%N MASTURBATIVE!", attackerClient);
				}
				case 7:
				{
					PrintCenterTextAll("%N IS MEGA FUCKAGE!", attackerClient);
				}
				case 8:
				{
					PrintCenterTextAll("%N GAYLIKE!!!!", attackerClient);
					stats_Gaylike(attackerClient);
				}
				
				
				case 9:
				{
					PrintCenterTextAll("DOUBLE PENETRATION by %N ", attackerClient);
					stats_dKills(attackerClient);
				}
				case 10:
				{
					PrintCenterTextAll("TRIPLE DICK PENETRATION by %N ", attackerClient);
					stats_tKills(attackerClient);
				}
				case 11:
				{
					PrintCenterTextAll("MULTI DICK INTO THE ASS by %N ", attackerClient);
					stats_mKills(attackerClient);
				}
				
			}
			if (filePosition > -1)
			{
				EmitSoundToAll(soundsFiles[filePosition], _, _, _, _, GetConVarFloat(cvarVolume));
			}
		}
	}
	else
