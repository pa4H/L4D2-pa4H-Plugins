...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_video", Cmd_Video);
	RegConsoleCmd("sm_online", Cmd_Online);
	RegConsoleCmd("sm_players", Cmd_Online);
	...
	LoadTranslations("pa4HVertigo.phrases");
	
	FormatEx(PREFIX, sizeof(PREFIX), "%t", "PREFIX");
	AddCommandListener(blockcmd, "go_away_from_keyboard");
}

public Action blockcmd(iClient, const char[] sCommand, iArg)
{
...
public OnGameEvent(Handle:event, const String:name[], bool:none)
{
	if (StrEqual(name, "bot_player_replace"))
	{
		...
		{
			//CPrintToChat(client, "%t", "SuspectCheater", PREFIX);
...
public OnEntityOutput(const String:output[], entity, client, Float:delay)
{
	...
	else if (StrEqual(sEntName, "trigger_start") && !IsFakeClient(client))
	{
		...
		CPrintToChat(client, "%t", "HelpMeTpAndSave", PREFIX);
		CPrintToChat(client, "%t", "HelpMePlay");
...
public void helpToConsole(int client)
{
...
public Action:Cmd_JoinSurvivor(client, args)
{
	if (client > 0 && GetClientTeam(client) != 2)
	{
		...
		{
			CPrintToChat(client, "%t", "ErrorCmd", PREFIX);
		}
	}
	return Plugin_Handled;
}
...
public Action:Cmd_Video(client, args)
{
	if (client > 0)
	{
		CPrintToChat(client, "%t", "HelpVideo");
	}
	return Plugin_Handled;
}
...
public Action:Cmd_Online(client, args)
{
	if (client > 0) {
		...
		{
			if (IsValidClient(i)) {
				int tim = RoundToFloor(GetClientTime(i)) / 60;
				CPrintToChat(client, "{olive}%N  {default}|  {lightgreen}Время: %i мин.", i, tim);
			}
		}
	}
	return Plugin_Handled;
